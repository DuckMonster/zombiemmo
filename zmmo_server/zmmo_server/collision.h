...
template<typename T>
bool QueryCollision( CGame* game, glm::ivec2 position, collision::HitResult<T>* outResult = NULL, unsigned int typeMask = 0xFFFFFFFF, unsigned int collisionMask = collision::Solid ) {
	bool result = false;

	CMap* map = game->GetMap( );
	CChunk* chunk = game->FindChunk( position );

	if ((typeMask & collision::Map) != 0)
		if ((map->GetTile( position ).m_Collision & collisionMask) != 0)
			result = true;

	if ((typeMask & collision::Entity) != 0 && chunk != NULL) {
		const std::vector<entity_id> entityList = chunk->AllEntities( );
